...
SRC_URI="mirror://sourceforge/freepascal/Source/fpcbuild-${PV}.tar.gz
	mirror://sourceforge/freepascal/Source/fpc-${PV}.source.tar.gz
	amd64? ( mirror://sourceforge/freepascal/Linux/3.2.0/${P}-x86_64-linux.tar )
	x86? ( mirror://sourceforge/freepascal/Linux/3.2.0/${P}.i386-linux.tar )
...
src_install() {
	...
	# A .so file ends up in /usr/lib for some reason
	if [ ${ARCH} == "amd64" ]; then
		mkdir -v "${ED}"/usr/lib64
		mv -v "${ED}"/usr/lib/*.so "${ED}"/usr/lib64
	fi
}
